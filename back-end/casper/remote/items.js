const getItems = function() {
  ...
  const getPicture = function(element) {
    if (element && select(element, 'span.lazyload')[0]) {
      return select(element, 'span.lazyload')[0].getAttribute('data-imgsrc');
    }
    return '';
  };

  return select(document, 'section.tabsContent > ul > li > a').map(function(
    element
  ) {
    return {
